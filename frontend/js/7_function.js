...
// 5. 실습
// 5 - 1. 표준함수 형태로 매개변수 2개를 입력받아서 더하는 함수, add2
function add2(x, y) {
    return x + y;
}

// 5 - 2. 위 함수를 상수 add3의 이름으로 값을 받도록 익명함수로 변환
const add3 = function (x, y) {
    return x + y;
}

// 5 - 3. 위 함수를 상수 add4의 이름으로 값을 받도록 람다함수로 변환
const add4 = (x, y) => { 
    return x + y;
}

// 5 - 4. 위 함수를 상수 add5의 이름으로 값을 받도록 생략가능한 부분을 모두 생략
const add5 = (x, y) => x + y;

// 5 - 5. 매개 변수가 1개라면? 화살표함수에서 () 생략 가능함
// 아래 두개의 함수는 같은 함수
const cal4 = (x) => x * 5;
const cal5 = x => x * 5;

// 5 - 6. 함수가 객체를 반환한다면? (참고)
